<?php $title = "Vêtements - L'annuaire éco-responsable"; ?>

<?php ob_start(); ?>
<div class="tout">
    <section class="titre_page_vetements">
        <h3>Retrouvez des marques de vêtements éco-responsables.</h3>
    </section>
...
$valeur_avis='null'; ?>
        <ul class="liste_marques">
            <?php while ($data = $avisparmarqueVetements->fetch()) { ?>
                <!-- 
--- LA MARQUE et infos 
-->
    ...
    /* NE PAS CHANGER : on remet la bonne valeur pour la suite */
        $valeur_marque = $data["nom_marque"]; ?>
            ...
            <?php    
}$avisparmarqueVetements->closeCursor();
?>
        </ul>
    </section>
...
AVIS-->
        <div class="background_avis">
            <div class="avis_dela_marque">
                <div class="fermer_avis_marque_top"><a href="index.php?action=pageVetements"><i class="fas fa-times"></i> Fermer</a></div>
...
?>
        ...
        if ($page > 1):
        ?><a id='page=1' href="index.php?action=pageAvisVetements&page=<?php echo $page - 1; ?>&amp;marqueM=<?php echo $marqueM ?>">Page précédente</a> — <?php
        endif;
        ...
        ?><a href="index.php?action=pageAvisVetements&page=<?php echo $i; ?>&amp;marqueM=<?php echo $marqueM ?>"><?php echo $i; ?></a> <?php
        endfor;

        /* Avec le nombre total de pages, on peut aussi masquer le lien
         * vers la page suivante quand on est sur la dernière */
        if ($page < $nombreDePages):
        ?>— <a href="index.php?action=pageAvisVetements&page=<?php echo $page + 1; ?>&amp;marqueM=<?php echo $marqueM ?>">Page suivante</a><?php
        endif;
        ?>
                    <div class="fermer_avis_marque_bottom"><a href="index.php?action=pageVetements"><i class="fas fa-times"></i> Fermer</a></div>
                </div>
            </div>
        </div>
    </section>
