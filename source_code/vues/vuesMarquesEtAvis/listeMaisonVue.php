...
<section class="titre_page_vetements">
    <h3>Retrouvez des marques pour la maison éco-responsables.</h3>
</section>
...
-->
    <div class="laisser_avis">
        ...
        <form action="index.php?action=publiAvisMaison&amp;marqueM=<?= $data2['nom_marque'] ?> " class="form_laisser_avis" method="post">
    ...
    if ($valeur_marque != $data["nom_marque"]) { /*si different -> rupture */ ?>
            ...
            if ($data["0dechet"] == 1){ ?>
                        ...
                        <div class="actions_une_marque">
                            <?php
                    if ($data["note_avis"] != ""){ ?>
                            <a href="index.php?action=pageAvisMaison&amp;marqueM=<?= $data['nom_marque'] ?>" class="display_avis">Afficher les avis</a>
                            <?php } else { ?>
